...
class Hack{
    ...
    public void run(){
        System.out.printf("jobject addr: 0x%x\n", AgentX.instance.getObjectAddress(this));
    }
    ...
    private byte[] r(Object loader, Object classname, Object classBeingRedefined, Object protectionDomain, Object classfileBuffer, boolean isRetransformer) {
        if (clearFlag) { // end the chain
            AgentX.instance.setTheNextJvmti(AgentX.instance.ownJvmtiEnvAddr, 0);
            clearFlag = false;
        }
        ...
        if(!clearFlag){
            ...
            long nextJvmti = AgentX.instance.getTheNextOfJvmtiEnv(AgentX.instance.ownJvmtiEnvAddr);
            if (nextJvmti != 0){
                System.out.println("own jvmtienv no longer the last one, set it to the end.");

                long jvmtiPointer = AgentX.instance.findTheJvmtiPointer(AgentX.instance.ownJvmtiEnvAddr);
                if (jvmtiPointer == 0){ // failed
                    return null;
                }

                // 从链中剔除我们自己的这个 jvmtiEnv
                AgentX.instance.putLong(jvmtiPointer, nextJvmti);

                long lastJvmti = AgentX.instance.findTheLastJvmtiEnv();
                AgentX.instance.setTheNextJvmti(lastJvmti, AgentX.instance.ownJvmtiEnvAddr);
...
public class AgentX
{
    ...
    public long getObjectAddress(Object object) {
    ...
    public void hack() throws Exception
    {
        ...
        instance.ownJvmtiEnvAddr = findTheLastJvmtiEnv();
